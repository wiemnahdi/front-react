...
import AddForm from '../add_crud/AddFormationForm';
import ShowPopupDetails from '../show/ShowPopupFormationDetails';
import EditPopupDetails from '../edit_crud/EditPopupFormationDetails';
import DeleteConfirmationPopup from '../delete_crud/DeleteFormationConfirmationPopup';
...
function CrudPage() {
  ...
  const handleAdd = async (nom) => {
    ...
    try {
      const response = await axios.post(`${server_uri}/formation`, { nom }, {
        headers: {
  ...
  const handleUpdateData = async (id, nom) => {
    ...
    setData(newData);
    try {
      await axios.put(`${server_uri}/formation/${id}`, { nom }, {
        headers: {
          Authorization: `Bearer ${currentUser}`
        },
      });
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await axios.delete(`${server_uri}/formation/${itemToDelete.id}`, {
        headers: {
          Authorization: `Bearer ${currentUser}`
        },
      });
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const response = await axios.get(`${server_uri}/formation`, {
          headers: {
            Authorization: `Bearer ${currentUser}`
          },
        });
